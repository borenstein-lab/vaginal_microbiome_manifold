...
for (data in append(unique(meta$db), "All")){
  ...
  # curr_s = get_bv_to_ps(db_meta, db_meta$mt_pseudotime, db_meta$shannon_index, db_meta$subCST, 'Pseudo-time', 'Shannon Diversity Index', y ~ poly(x, 3, raw = TRUE), 'subCST', c("R2", "p.value"))
  # db_plots_lst[[d]] = curr_s
...
write_xlsx(db_df, paste0("C:/Users/morts/Documents/TAU/Drafts/PAGA_paper/", "blocks_res.xlsx"))
```
```{r}
# library(lme4)
# 
# rmodel = lmer(shannon_index ~ mt_pseudotime + (1 | db), data = meta)
# print(summary(rmodel))
# print(coef(rmodel)$db)
