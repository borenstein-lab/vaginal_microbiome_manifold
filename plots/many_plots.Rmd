...
get_bv_to_ps = function(df, x, y, fill_col, x_tit, y_tit, formula, legend_name, stat_label, colors_vec = community_colors, facet = TRUE){
  ...
  # stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")), method = "spearman", label.y.npc="top", label.x.npc = "left", size = 6, family = 'Gafata') +
...
# tmp = tmp %>% group_by(subjectID) %>% filter(sum(menst) >= 1)
arr0 = menst_df %>% filter(sum_all == 0) %>% dplyr::select(ps_diff)  %>% drop_na() %>% pull()
arr1 = tmp %>% filter(sum_all == 1) %>% dplyr::select(ps_diff)  %>% drop_na() %>% pull()
wilcox.test(arr0, arr1, paired = FALSE, alternative = "less")

m = ggplot(data = tmp, 
           aes(x = factor(sum_all), y = (log_ps_diff)), color = 'slategray4') + #log(ps_diff)
...
print(m)
ggsave(filename = paste0(save_path, 'menst_delta_14102023.png'), plot = m,
...
p1 = get_umap_subject(all_meta, all_meta, 'UAB103', 0.9, 3, 30, 0.5, 0) +
    labs(y = 'UMAP') +
    annotate(geom = 'text', label = 'a', x = -Inf, y = Inf, hjust = -1, vjust = 1.5, family = 'Gafata', size = 7) +
    theme(axis.title = element_text(size = 42, color = 'white'))
p2 = get_umap_subject(all_meta, all_meta, 'UAB059', 0.9, 3, 30, 0.5, 0) +
     annotate(geom = 'text', label = 'b', x = -Inf, y = Inf, hjust = -1, vjust = 1.5, family = 'Gafata', size = 7) 
p3 = get_umap_subject(all_meta, all_meta, 'UAB002', 0.9, 3, 30, 0.5, 0) +
     annotate(geom = 'text', label = 'c', x = -Inf, y = Inf, hjust = -1, vjust = 1.5, family = 'Gafata', size = 7) 
...
prog_df = prog_df + 
  ...
  guides(color = guide_legend(override.aes = list(shape = 19, size = 2), nrow = 2, title.hjust = 0.1, title.vjust = 1))

p_umap = plot_grid(p1, p2, p3, ncol = 3, align = "vh")
pall = plot_grid(p_umap, prog_df, ncol = 1, align = "v", rel_heights = c(.6, 1), rel_widths = c(1, .5))

ggsave2(
  paste0(save_path, "UMAP_subjectID_example_30102023.png"),
  plot = pall,
  device = NULL,
  path = NULL,
  scale = 1,
