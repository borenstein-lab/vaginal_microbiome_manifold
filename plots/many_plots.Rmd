...
health_meta = temp_meta %>% filter(CST %in% c('I', 'II', 'III', 'V'))
s = get_bv_to_ps(health_meta, health_meta$mt_pseudotime, health_meta$shannon_index, health_meta$subCST, 'Pseudo-time', 'Shannon Diversity Index', y ~ poly(x, 3, raw = TRUE), 'subCST', c("R2"), community_colors)
...
m = ggplot(data = tmp, 
  ...
  labs(x = '', y = "Pseudo-time differentiation (log)") +
...
print(m)
# ggsave(filename = paste0(save_path, 'menst_delta_17072023.png'), plot = m,
...
```{r}
db_colors = c('UAB_pyro' = '#006d77', 'UAB_hiseq' = '#83c5be', 'VCU' = '#edf6f9', 'AVPVC' = '#ffddd2', 'PVI' = '#E6744A', 'PHS' = '#BF5F3C')
plot_heatmap <- function(df, vec_cst, vec_score, n = 25) {
  ...
  column_ha = HeatmapAnnotation(subCST = all_df$subCST,
                                  Score = anno_barplot(all_df$score, baseline = 0, height = unit(0.3, "cm"),
                                                       gp = gpar(col = "#462255"), axis_param = list(at = c(0, 1))),
                                  Dataset = all_df$db,
                                  col = list(subCST = community_colors, Dataset = db_colors),
                                  annotation_legend_param = list(subCST = list(nrow = 1),
                                                                 Dataset = list(nrow = 1)))
  h = Heatmap(plot_mat, name = 'Abundance', col = col_fun, 
              show_column_names = FALSE,
              top_annotation = column_ha, 
              cluster_rows = FALSE, cluster_columns = FALSE,
              # row_names_max_width = max_text_width(rownames(d), 
              row_names_gp = gpar(fontsize = 10),
              heatmap_legend_param = list(legend_direction = "horizontal", legend_width = unit(70, "mm")))
  return(h)
  }
```
...
print(h)
png(paste(top_path, 'all_df_02072023.png', sep = ''), 
...
v = ggplot(data = meta, aes(x = subCST, y = shannon_index, fill = subCST)) +
    geom_boxplot() +
    ...
    theme(text = element_text(size = 27)) +
    guides(fill=guide_legend(ncol=2))
ggsave(paste0(top_path, 'v.png'), height = 7, width = 12, units = "in", dpi = 150)
# print(v)
```
